...
(define enumerate-tree$
  (lambda(tree cont)
    (cond
      ...
      [else (enumerate-tree$ (cdr tree)
                             (lambda(x) (append (cont x) (list (car tree)))))])))


;Signature: replace+$ (expr, succ-cont, fail-cont)
;Type:      [List U Symbol * [T -> T] * [Empty -> List union Symbol] -> List U Symbol]
;Purpose:   Replace the leftmost occurrence of the symbol `*` in expr by the symbol `+`
;Tests:     (replace+$ (make-proc (make-tuple-te (list 'Number make-proc(make-tuple-te(list 'T1) 'T2)) 'T1)))
;             ==> '(-> (+ num (-> (* T1) T2)) T1)
(define replace+$
  (lambda (expr succ-cont fail-cont)
    (cond
      [(null? expr) (fail-cont)]
      [(not (list? expr)) (if (eq? expr '*) (succ-cont '+) (fail-cont))]
      [else (replace+$ (car expr)
                       (lambda (car-res) (succ-cont (cons car-res (cdr expr))))
                       (lambda ()
                         (replace+$ (cdr expr)
                                    (lambda (cdr-res)
...
;Signature: replace->$ (expr, succ-cont, fail-cont)
;Type:      [List U Symbol * [T -> T] * [Empty -> List U Symbol] -> List U Symbol]
...
(define replace->$
  (lambda (expr succ-cont fail-cont)
    (cond
      ...
      [else (replace->$ (car expr)
                        (lambda (car-res) (succ-cont (cons car-res (cdr expr))))
                        (lambda () (replace->$ (cdr expr)
                                               (lambda (cdr-res) (succ-cont (cons (car expr) cdr-res)))
                                               fail-cont)))])))
   ...
   (cond
     [(not (list? expr))
      (cond [(eq? expr '*) '+]
            [(eq? expr '->) '=>]
            [else expr])]
    [else (replace+$ (reverse$ (replace->$ (reverse$ expr)
                                           (lambda (x) x)
                                           (lambda() (expr))))
                     (lambda (x) x)
                     (lambda() (expr)))])))
...
(define reverse$ (lambda(lst)
  (letrec ([iter$
             (lambda (lst cont)
               (cond
                 [(null? lst) (cont '())]
                 [(list? (car lst)) (iter$ (cdr lst)
                                           (lambda (new-tail) (iter$ (car lst)
                                                                     (lambda (new-head) (cont (append new-tail (list new-head)))))))]
                 [else (iter$ (cdr lst)
                              (lambda (new-tail) (cont (append new-tail (list (car lst))))))]))])
